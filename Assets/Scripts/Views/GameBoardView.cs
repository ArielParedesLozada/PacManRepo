...
public class GameBoardView : MonoBehaviour, IGameBoardGateway
{
	...
	public static bool isCurrentPlayerUp = true;
	...
	void Start()
	{
		currentPlayerLevel = new LevelSetter().GetLevel();
	...
	void SpawnBonusItemForPlayer(int playernum)
	{
		...
		if (playernum == 1)
		{
			...
			if (GameMenu.currentPlayerPelletsConsumed >= 70 && GameMenu.currentPlayerPelletsConsumed < 170)
			{
				...
				if (!didSpawnBonusItem1_player1)
				{
					...
					SpawnBonusItemForLevel(currentPlayerLevel);
				}

			}
			else if (GameMenu.currentPlayerPelletsConsumed >= 170)
			{
	...
	void UpdateUI()
	{
		...
		currentPlayerScoreText.text = currentPlayerScore.ToString();
		...
		if (isCurrentPlayerUp)
		{
			...
			if (GameMenu.livesCurrentPlayer == 3)
			{
			...
			else if (GameMenu.livesCurrentPlayer == 2)
			{
			...
			else if (GameMenu.livesCurrentPlayer == 1)
			{
	...
	void CheckPelletsConsumed()
	{
		...
		if (totalPellets == GameMenu.currentPlayerPelletsConsumed)
		{
	...
	void PlayerWin(int playerNum)
	{
		...
		if (playerNum == 1)
		{
			...
			if (!didIncrementLevel)
			{
				...
				currentPlayerLevel++;
				new LevelSaver().Save(currentPlayerScore, currentPlayerLevel);
	...
	private void StartNextLevel()
	{
		...
		if (isCurrentPlayerUp)
		{
			...
			GameMenu.currentPlayerPelletsConsumed = 0;
	...
	IEnumerator ProcessStartNextLevel(float delay)
	{
		...
		if (isCurrentPlayerUp)
			StartCoroutine(StartBlinking(currentPlayerUp));
	...
	public void StartDeath()
	{
		if (!didStartDeath)
		{
			...
			if (GameMenu.isOnePlayerGame)
			{
				currentPlayerUp.GetComponent<Text>().enabled = true;
			}
			else
			{
				currentPlayerUp.GetComponent<Text>().enabled = true;
	...
	IEnumerator ProcessRestart(float delay)
	{
		...
		if (isCurrentPlayerUp)
			GameMenu.livesCurrentPlayer -= 1;
		if (GameMenu.livesCurrentPlayer == 0 && GameMenu.livesCurrentPlayer == 0)
		{
		...
		else if (GameMenu.livesCurrentPlayer == 0 || GameMenu.livesCurrentPlayer == 0)
		{

			if (GameMenu.livesCurrentPlayer == 0)
			{
			...
			if (!GameMenu.isOnePlayerGame)
				isCurrentPlayerUp = !isCurrentPlayerUp;
